...
영업사원 포털 → 사업주콘솔 가입 → 어드민 검수/승인 → 사업주콘솔 로그인 → 홈페이지 상품 노출의 전체 흐름을 검증한다.

## 사전 조건

1. `20260804110000_create_partner_onboarding.sql` 마이그레이션을 적용한다.
2. 홈페이지, 어드민/영업 포털, 사업주콘솔이 동일한 Supabase 프로젝트를 사용한다.
...
- 승인 전 사업주콘솔 로그인은 `가입 승인을 기다리고 있습니다`로 차단된다.
